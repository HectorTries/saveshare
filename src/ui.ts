/* ---------- DOM UI layer: debt editor + top HUD.
   The game loop itself (pay → meter → push → readout) lives
   entirely in the Phaser scenes; DOM only handles setup and the
   persistent top bar. ---------- */
...
function renderDebtList(): void {
  ...
  store.debts.forEach((d, i) => {
    ...
    if (d.paid > 0) {
      ...
      paidRow.textContent = `✓ ${money(d.paid)} paid so far (${Math.round(pctPaid(d) * 100)}% of the slope) — progress you keep`;
...
function addDebtRow(): void {
  store.debts.push({ name: '', balance: 0, apr: 19.9, months: 36, paid: 0, progress: 0, terrain: [], celebrated: 0 });
  renderDebtList();
...
export function initUI(): void {
  ...
  $('resetBtn').addEventListener('click', () => {
    if (confirm('Reset ALL payment progress? Every mountain returns to its peak.')) {
      store.resetProgress();
      hideOverlays();
      bus.emit('start');
    }
